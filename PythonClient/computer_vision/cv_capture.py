...
for x in range(50): # do few times
    ...
    responses = client.simGetImages([
        ...
        airsim.ImageRequest("1", airsim.ImageType.Scene, compress=False),
        airsim.ImageRequest("2", airsim.ImageType.Scene, compress=False)])
    ...
    for i, response in enumerate(responses):
        if response.pixels_as_float:
            ...
            # print("Type %d, size (%d, %d, %f), pos %s" % (response.image_type, W, H, C, pprint.pformat(response.camera_position)))
