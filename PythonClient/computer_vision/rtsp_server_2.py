...
class GstServer(object):
	...
	def __init__(self):
		...
		self.factory.set_launch("( appsrc name=AirSimRtspSrv ! videoconvert ! x264enc ! rtph264pay name=pay0 pt=96 )")
	...
	def on_media_configure(self, media):
		element = media.get_element()
		appsrc = element.get_by_name_recurse_up("AirSimRtspSrv")

		appsrc.set_property("format", Gst.Format.TIME)
		appsrc.set_caps(Gst.Caps.from_string("video/x-raw,rate=30,width=256,height=144,format=I420"))
		appsrc.connect("need-data", self.need_data_cb)
